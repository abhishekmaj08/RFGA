### 19/04/2026
### Same as 4_runGA_Normal_vs_Tumor_parallel except:
### updated spatial scoring system. Add more components.
### lets see if there are any improvements
...
overlapWithTruth<-function(Truth,Prediction){ ##size of Prediction is more than that of Truth. First delete the extra spots then reorder and compare
  score = list()
  ...
  tIndx2 = which(pred$Predicted=="Tumor")
  pred_tumor = pred[tIndx2,]
  pred_normal = pred[-tIndx2,]
  
  score_tumor = length(intersect(Truth_tumor$X,pred_tumor$Barcode))
 
  score_normal = length(intersect(Truth_normal$X,pred_normal$Barcode))
  
  print(paste0("Normal overlap: ",score_normal/nrow(Truth_normal)," Tumor overlap: ",score_tumor/nrow(Truth_tumor)))
  
  score[[1]] = score_normal/nrow(Truth_normal)
  score[[2]] = score_tumor/nrow(Truth_tumor)
  
  #print(paste0("i: ",i," ",pred$Barcode[i]," ",Truth$X[i]," ",pred$Predicted[i]," ",Truth$value[i]," ",pred$Predicted[i]==Truth$value[i]," Score: ",score))
...
# barcodes=c("a1","a2","a3","a4","a5","a6","a7","a8","a9","a10","b1","b2","b3","b4","b5","b6","b7","b8","b9","b10")
# Truth = data.frame(X=barcodes)
# Truth$value = "Tumor"
# Truth$value[11:20] = "Normal"
# Prediction = data.frame(Barcode=barcodes)
# Prediction$Predicted = "Normal"
# Prediction$Predicted[sample(c(1:20), 14)] = "Tumor"
...
getChromosomeFitnessScore<-function(chrom){
  ...
  degenesIndx<-which(rownames(count_stand) %in% deGenes)
  GE = count_stand[degenesIndx,]
  pcaInput<-t(GE)
  ...
  ##sanity check
  all(rownames(data3)==ww2$ID)
  data3$Type = factor(ww2$Type)
  ...
  #spat_score[[1]] = correctly_classified_truthNormal/(nrow(Truth_normal)+epsilon)
  #spat_score[[2]] = incorrectly_classified_truthNormal/(nrow(Truth_normal)+epsilon)
  #spat_score[[3]] = correctly_classified_truthTumor/(nrow(Truth_tumor)+epsilon)
...
qq<-read.csv(file = paste0("THCA_ge_standardized_ver2.2.csv"), header = T)
rownames(qq)<-qq$X
count_stand<-qq[,-1]
tt<-read.csv(file = paste0("THCA_ge_standardized_annotation_ver2.2.csv"), header = T)
ww2<-tt
...
### to create the set of unused genes
thrshld = 0 ## *****
firstDEG<-read.csv(file = paste0("THCA_ge_basicStats_standardizedData_ver2.2.csv"), header = T)
firstDEG$pvalue_Adj = p.adjust(firstDEG$p.val_Tumor_Normal, method="BH")
deg2<-firstDEG$X[which(firstDEG$pvalue_Adj<pSigVal)]
tGenes = deg2 ## unused genes 
######


sotti = read.csv(file = paste0("M1_TruthForRFGA.csv"), header = T)

##change the barcode to match
bc = gsub("-",".",sotti$X)
TRUTH=data.frame(X=bc,value=sotti$value)

allSpatData<-read.csv(file = paste0("M1_rawData.csv"), header = T)
...
tcga_scores = read.csv(file = paste0("Updated_TCGA_Scores","_","2026-04-20",".csv"), header = T)

cutoff = 1.2 ## 20 April 2026

tcga_scores = tcga_scores[which(tcga_scores$FitnessScore3>cutoff),]
...
for(i in 1:nrow(tcga_scores)){
  if(tcga_scores$TumorThrshld[i]!=0){
    knownGood<-read.csv(file = paste0("deGenes_thrshld",tcga_scores$TumorThrshld[i],"_2025-10-28.csv"), header = T)
    ...
    fitnessScores[i] = tcga_scores$FitnessScore3[i]
    
    if(tcga_scores$FitnessScore3[i]>bestFitness)  {
      bestIndx = i
      bestFitness = tcga_scores$FitnessScore3[i]
      bestSize = tcga_scores$deGenes[i]
      bestSolnBA=tcga_scores$TCGABalancedAccuracy[i]##$TCGABalancedAccuracy
      bestSolnccTN=tcga_scores$ccTN[i] ##$correctly_classified_truthNormal
      bestSolnicTN=tcga_scores$icTN[i] ##$incorrectly_classified_truthNormal
      bestSolnccTT=tcga_scores$ccTT[i] ##$correctly_classified_truthTumor
      bestSolnicTT=tcga_scores$icTT[i] ##$incorrectly_classified_truthTumor
    } 
  }
}
...
bestClassifier<-readRDS(file = paste0("THCA_RF2_thrshld",tcga_scores$TumorThrshld[bestIndx],"_2025-10-28.rds"))
...
while(noImprovementInBest<terminationCondition){
  ...
  discardOffspring<-rep(0,length(offspring))  ## vector to keep track of offspring viability
  alreadySeen<-rep(0,length(offspring))  ## vector to keep track whether generation already present in population
...
ss<-sort.int(fitnessScores, decreasing = T, index.return = T)
...
write.csv(df11, file = paste0("NormalTumor_GA_Performance_SolnSize=",length(bestSoln),"_",date,".csv"), row.names = F)
df12<-data.frame(deGenes=bestSoln)
write.csv(df12, file = paste0("NormalTumor_GA_deGenes_SolnSize=",length(bestSoln),"_",date,".csv"), row.names = F)
saveRDS(bestClassifier, file = paste0("NormalTumor_GA_Classifier_SolnSize=",length(bestSoln),"_",date,".rds"))
