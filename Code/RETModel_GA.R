...
getChromosomeFitnessScore<-function(chrom){
  ...
  fitControl <- trainControl(method = "LOOCV", # Leave-One-Out is great for N < 100
...
if(interest == "RET.fusion") idate = "2026-04-30"

pSigVal = 0.01

basicStats<-read.csv(file = paste0("8",interest,"-vs-Normal-basicStats_",idate,".csv"), header = T)
...
tcga_scores = read.csv(file = paste0("8",interest,"_vs_Normal_DifferentThresholds_",idate,".csv"), header = T)
...
if(interest == "RET.fusion") cutoff = 0.0
...
for(i in 1:nrow(tcga_scores)){
  ...
  knownGood<-read.csv(file = paste0("deGenes_8",interest,"_vs_Normal_thrshld",tcga_scores$TumorThrshld[i],"_",idate,".csv"), header = T)
  temp<-knownGood$deGenes
  
  #classifier<-readRDS(file = paste0(prefix,interest,"_vs_Normal_RF2_thrshld",tcga_scores$TumorThrshld[i],"_",idate,".rds"))
...
bestClassifier<-readRDS(file = paste0("8",interest,"_vs_Normal_RF2_thrshld",tcga_scores$TumorThrshld[bestIndx],"_",idate,".rds"))
...
iteration<-1
terminationCondition = 1000 #noImprovementInBest for 1000 iterations
